...
const servicesFormCategoryDropdown = document.querySelector(
...
const reviewModalInput = document.querySelector(
  ".people-saying-review-modal__container input"
);
const reviewModalTextarea = document.querySelector(
  ".people-saying-review-modal__container textarea"
);
...
document.addEventListener("click", (event) => {
  ...
  if (findParent(event, "header__menu-burger", 2, true)) {
    ...
    !findParent(event, "header__menu", 3, true) &&
    headerMenu.classList.contains("header__menu_open")
  ) {
  ...
  if (
    findParent(event, "people-saying-card", 3, true) &&
    findParent(event, "swiper-slide", 3, true)
  ) {
    wrapper.classList.add("open_people-saying-modal");
  }
  // Close people-saying-modal
  if (
    findParent(event, "people-saying-card__close-btn", 2, true) ||
    event.target.classList.contains("people-saying-modal")
  ) {
  ...
  if (findParent(event, "faq__question", 3, true)) {
    ...
    if (event.target.classList.contains("faq__question")) {
      ...
      event.target.parentNode.parentNode.classList.contains("faq__question")
    ) {
    ...
    const activeQuestion = document.querySelector(".faq__question_open");

    if (activeQuestion) {
      activeQuestion.classList.remove("faq__question_open");
    }
  ...
  if (findParent(event, "faq__form-dropdown-btn", 2, true)) {
    ...
    !findParent(event, "faq__form-dropdown", 4, true) &&
    document.querySelector(".faq__form-dropdown_open")
  ) {
    faqFormDropdown.classList.remove("faq__form-dropdown_open");
  }
  ...
  if (findParent(event, "services__form-category-btn", 2, true)) {
    servicesFormCategoryDropdown.classList.toggle(
      "services__form-category_open"
    );
  } else if (event.target.id.includes("services-category-")) {
    let number = document.querySelectorAll(
      ".services__form-category-items ul li input:checked"
    ).length;
    document.querySelector(
      ".services__form-category-items-info-selected span"
    ).textContent = number;
  } else if (
    event.target.classList.contains("services__form-category-items-info-clear")
  ) {
    let checkedElems = document.querySelectorAll(
      ".services__form-category-items ul li input:checked"
    );
    if (checkedElems) {
    ...
    !findParent(event, "services__form-category", 4, true) &&
    document.querySelector(".services__form-category_open")
  ) {
    servicesFormCategoryDropdown.classList.remove(
      "services__form-category_open"
    );
  }
  ...
  if (findParent(event, "services__form-sorting-btn", 2, true)) {
    ...
    !findParent(event, "services__form-sorting-items", 1, true) &&
    document.querySelector(".services__form-sorting_open")
  ) {
    servicesFormSorting.classList.remove("services__form-sorting_open");
  }
  ...
  if (event.target.id.includes("influencer-")) {
    ...
    document.querySelector(".service-info__influencers > p").innerHTML =
      "Influencer:<span></span>";
    document.querySelector(".service-info__influencers > p span").textContent =
      name;
  }
  ...
  if (event.target.id.includes("coverage-")) {
    ...
    document.querySelector(".service-info__coverage > p").innerHTML =
      "User coverage:<span></span>";
    document.querySelector(".service-info__coverage > p span").textContent =
      name;
  }
  ...
  if (findParent(event, "service-info__dropdown-btn", 2, true)) {
    ...
    let allPackages = serviceInfoDropdown.querySelectorAll(
      ".service-info__dropdown-items div"
    );
    allPackages.forEach((package, index) => {
      if (package.querySelector("input") == event.target) {
        ...
        let btn = serviceInfoDropdown.querySelector(
          ".service-info__dropdown-btn span"
        );
        btn.textContent = `Package ${index + 1} : ${packageText}`;
        btn.style.opacity = 1;
        document
          .querySelector(".service-info__btns .add-to-cart-btn")
    ...
    !findParent(event, "service-info__dropdown", 4, true) &&
    document.querySelector(".service-info__dropdown_open")
  ) {
    serviceInfoDropdown.classList.remove("service-info__dropdown_open");
  }
  ...
  if (
    event.target.classList.contains("people-saying-review__write-review-btn")
  ) {
  ...
  if (findParent(event, "people-saying-review-modal__stars", 2, false)) {
    ...
    document
      .querySelectorAll(".people-saying-review-modal__stars svg")
      .forEach((star) => {
      ...
      .forEach((star, index) => {
        if (currentStar == star) {
          for (let i = 1; i <= index + 1; i++) {
            document
              .querySelector(
                `.people-saying-review-modal__stars svg:nth-child(${i})`
              )
  ...
  if (
    findParent(event, "people-saying-review-modal__close-btn", 2, true) ||
    event.target.classList.contains("people-saying-review-modal") ||
    event.target.classList.contains("people-saying-review-modal__btn")
  ) {
...
function checkUserReview() {
  if (
    ...
    document
      .querySelector(".people-saying-review-modal__btn")
      .removeAttribute("disabled");
  }
}
...
const additionalServices__Swiper = new Swiper(
  ".best-offers__swiper_additional-services",
  {
    spaceBetween: 40,
    slidesPerView: "auto",
    resistanceRatio: 0,
